...
class ProviderManager {
  ...
  search(...args) {
    const [selectedProviders, query, category, limit, sort] = this.handleSearchArguments(...args);

    const searchPromises = selectedProviders
      .map(p => p.search(query || '', category, limit, sort && sort !== 'seeds' ? sort : 'seeders'))
    ...
    return Promise.all(searchPromises)
      .then(results => {
        ...
        results = results.map(item => ({
          ...item,
          seeds: typeof item.seeds === 'undefined' ? 0 : item.seeds
        }));
        ...
        if (sort === 'time') {
          ...
          return _(results)
            .orderBy([sort ?? 'seeds', 'title'], ['desc', 'asc'])
            .value();
        }
      })
      .then(results => (limit ? results.slice(0, limit * selectedProviders.length) : results));
  }
